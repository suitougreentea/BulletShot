...
import jp.dip.suitougreentea.BulletShot.state.*;
...
public class GameBulletShot extends StateBasedGame {
	...
	@Override
	   protected void postRenderState(GameContainer container, Graphics g) throws SlickException {
	       ...
	       Res.debugfont.drawRight(String.format("Mem:%.1f/%.1f(%.1f)",
	               Runtime.getRuntime().freeMemory()/1048576f,
	               Runtime.getRuntime().totalMemory()/1048576f,
	               Runtime.getRuntime().maxMemory()/1048576f
	               ), 624, 16);
	       Res.debugfont.draw("Offline",16,452);
	       Res.debugfont.drawRight(BulletShot.getVersionShorter(),624, 452);
	        BulletShot.drawLog();
	   }
	   }
	   ...
	   protected void preUpdateState(GameContainer container, int delta){
	       if(stateselect){
	           ...
	           if(i.isKeyDown(Input.KEY_RSHIFT)){
	               if(i.isKeyPressed(Input.KEY_1))this.enterState(1);
	               if(i.isKeyPressed(Input.KEY_2))this.enterState(2);
	               if(i.isKeyPressed(Input.KEY_3))this.enterState(3);
	               if(i.isKeyPressed(Input.KEY_4))this.enterState(4);
	               if(i.isKeyPressed(Input.KEY_5))this.enterState(5);
	               if(i.isKeyPressed(Input.KEY_6))this.enterState(6);
	               if(i.isKeyPressed(Input.KEY_7))this.enterState(7);
	           }
	       }
	   }
