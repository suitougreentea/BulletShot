...
public class Stage {
    ...
    protected ObjectPlayer player;

    private float heightCoefficient = 1.340625f;
    ...
    public void initPhysics() {
        ...
        for (int iz = 0; iz < terrain.length; iz++) {
            for (int ix = 0; ix < terrain[0].length; ix++) {
                ...
                if (t.getSlopeRigidBody() != null) {
                    dynamicsWorld.addRigidBody(t.getSlopeRigidBody(), (short) 0x1, (short) 0x2);
                }
            }
        }
    ...
    public void shoot(int poweri, int rot, int vrot) {
        ...
        // fallRigidBody.applyForce(new Vector3f(x,325,y),new
        // Vector3f(x,325,y));
        // playerRigidBody.setLinearVelocity(new Vector3f(x,5,z));
        playerRigidBody.setLinearVelocity(new Vector3f(x * power * vp, heightCoefficient * power * vy, z * power * vp));
