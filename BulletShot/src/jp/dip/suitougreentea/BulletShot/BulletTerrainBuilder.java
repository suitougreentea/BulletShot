...
public final class BulletTerrainBuilder {
    private BulletTerrainBuilder() {
        ...
        for (int iz = 0; iz < terrain.length; iz++) {
            for (int ix = 0; ix < terrain[0].length; ix++) {
                ...
                if (t.getType() != Terrain.TERRAIN_NORMAL) {
                    ...
                    DefaultMotionState slopeMotionState = new DefaultMotionState(new Transform(new Matrix4f(new Quat4f(0, 0, 0, 1f), new Vector3f(ix, t.getHeight() * 0.5f, iz), 1f)));
                ...
                DefaultMotionState boxMotionState = new DefaultMotionState(new Transform(new Matrix4f(new Quat4f(0, 0, 0, 1), new Vector3f(ix + 0.5f, t.getHeight() * 0.25f, iz + 0.5f), 1f)));
