...
public class StateGame extends BasicGameState {
	...
	public void render(GameContainer gc, StateBasedGame sbg, Graphics gr)
	    ...
	    Res.dback.draw();
	    r.view3D();
	    if(state==STATE_SLEEPINGPHYSICS)r.draw(stage,predict,predicttimer);
	    else r.draw(stage,null,0);
        r.view2D();
        //gui.draw();
        ...
        if(state == STATE_SHOOT){
            ...
            int p;
            if(shootpowertimer > 64)p = 128-shootpowertimer;
            else p = shootpowertimer;
	...
	StageGenerator gen = new StageGeneratorFlat(20,20,1);
	Stage stage = new Stage(gen.generate(),0,0);
	PredictStage predictStage = new PredictStage(gen.generate(),0,0);
	
	Transform[] predict;
	
	int tHroti,tHrotd,tVroti,tVrotd;
	
	int state;
	public static final int STATE_SLEEPINGPHYSICS = 1;
	public static final int STATE_SHOOT = 2;
	public static final int STATE_WORKINGPHYSICS = 3;
	boolean cameraMoving;
	boolean powerDeciding;
	
	int shootpowertimer = 0;
	int shootwaittimer = 0;
    int hRot = 0;
    int vRot = 60;
	...
	public void initPhysics(){
	    ...
	    BulletShot.getLogger().info("Reset physics.");
	}
	...
	public void update(GameContainer gc, StateBasedGame sbg, int delta)
	    ...
	    if(state == STATE_SLEEPINGPHYSICS){
    		...
    		if(i.isKeyPressed(Input.KEY_X)) state = STATE_SHOOT;
