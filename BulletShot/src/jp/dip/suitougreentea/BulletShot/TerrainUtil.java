...
public final class TerrainUtil {
    ...
    public static float getHeight(float x, float z, int type, int height, int direction) {
        ...
        if (type == Terrain.TERRAIN_LOWSLOPE) {
            ...
            switch (direction) {
            case Terrain.DIRECTION_TOPLEFT:
                result = 1 - 0.5f * (x + z);
                break;
            case Terrain.DIRECTION_BOTTOMLEFT:
                result = 0.5f - 0.5f * (x - z);
                break;
            case Terrain.DIRECTION_BOTTOMRIGHT:
                result = 0.5f * (x + z);
                break;
            case Terrain.DIRECTION_TOPRIGHT:
                result = 0.5f + 0.5f * (x - z);
                break;
            }
            ...
            if (z <= 0.5f && z <= x && x <= 1 - z) {
                result = 0.5f * z; // TOP
            } else if (z >= 0.5f && 1 - z <= x && x <= z) {
                result = 0.5f * (1 - z); // BOTTOM
            } else if (x <= 0.5f && x <= z && z <= 1 - x) {
                result = 0.5f * x; // LEFT
            } else if (x >= 0.5f && 1 - x <= z && z <= x) {
                result = 0.5f * (1 - x); // RIGHT
            }
        }
